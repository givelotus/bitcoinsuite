...
use crate::{
    structs::{
        Amount, GenesisData, MintData, TokenType, BURN, GENESIS, MINT, SEND, SLPV2_LOKAD_ID,
    },
    token_id::TokenId,
};
...
pub fn sections_opreturn(sections: Vec<Bytes>) -> Script {
    Script::from_ops(
        [Op::Code(0x6a), Op::Code(0x50)]
            .into_iter()
            .chain(sections.into_iter().map(|section| Op::push_bytes(section))),
    )
